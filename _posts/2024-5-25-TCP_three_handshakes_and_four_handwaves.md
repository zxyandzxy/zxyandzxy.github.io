...
简单来说就是，**用于保证可靠性和流量控制而维护的某些状态信息，这些信息的组合，包括 Socket、序列号和窗口大小称为连接。**
...
从上面的过程可以发现**第三次握手是可以携带数据的，前两次握手是不可以携带数据的**，因为两次握手后，客户端和服务端就都知道了自己和对方的序列号和确认号。第三次握手只是为了确保连接。
...
**序列号和初始化序列号并不是无限递增的，会发生回绕为初始值的情况，这意味着无法根据序列号来判断新老数据**。如果 TIME-WAIT 没有等待时间或时间过短，被延迟的数据包抵达后会被新建立的连接接受，这样数据就混乱了。
