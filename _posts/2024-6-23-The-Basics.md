...
一个处于空闲状态的连接被服务端主动断开后，这个客户端并不会马上知道，等到客户端在发起下一个请求的时候，才会收到这样的报错“ERROR 2013 (HY000): Lost connection to MySQL server during query”。（**实现将获取信息的权利交给客户端，保证用户不会感到莫名其妙）**
...
优化器确定的不同执行方案，对于执行器来说就会有不同的执行流程，下面三种执行方案：
...
页的类型有很多，常见的有数据页、undo 日志页、溢出页等等。数据表中的行记录是用「数据页」来管理。表中的记录存储在「数据页」里面。
...
这条记录上一个版本的指针。roll_pointer 是必需的，占用 7 个字节。用于实现 MVCC
...
- 条件二：如果变长字段允许存储的最大字节数大于 255 字节，就会用 2 字节表示「变长字段长度」；(2^16 = 65536)
